...
struct DenseQpIpmSolver::WrapperHolder {
  WrapperHolder() 
    : dense_qp_dim_wrapper(std::make_shared<d_dense_qp_dim_wrapper>()),
      dense_qp_ipm_arg_wrapper(std::make_shared<d_dense_qp_ipm_arg_wrapper>()),
      dense_qp_wrapper(),
...
DenseQpIpmSolver::DenseQpIpmSolver(const DenseQp& dense_qp, 
...
DenseQpIpmSolver::DenseQpIpmSolver(const DenseQpIpmSolverSettings& solver_settings) 
  : solver_settings_(),
    solver_statistics_(),
    dim_(),
    wrapper_holder_(new WrapperHolder()) {
...
void DenseQpIpmSolver::setSolverSettings(const DenseQpIpmSolverSettings& solver_settings) {
  d_dense_qp_ipm_arg* dense_qp_ipm_arg_ptr = wrapper_holder_->dense_qp_ipm_arg_wrapper->get();
  ...
  d_dense_qp_ipm_arg_set_iter_max(&solver_settings_.iter_max, dense_qp_ipm_arg_ptr);
  d_dense_qp_ipm_arg_set_alpha_min(&solver_settings_.alpha_min, dense_qp_ipm_arg_ptr);
  d_dense_qp_ipm_arg_set_mu0(&solver_settings_.mu0, dense_qp_ipm_arg_ptr);
  d_dense_qp_ipm_arg_set_tol_stat(&solver_settings_.tol_stat, dense_qp_ipm_arg_ptr);
  d_dense_qp_ipm_arg_set_tol_eq(&solver_settings_.tol_eq, dense_qp_ipm_arg_ptr);
  d_dense_qp_ipm_arg_set_tol_ineq(&solver_settings_.tol_ineq, dense_qp_ipm_arg_ptr);
  d_dense_qp_ipm_arg_set_tol_comp(&solver_settings_.tol_comp, dense_qp_ipm_arg_ptr);
  d_dense_qp_ipm_arg_set_tol_dual_gap(&solver_settings_.tol_dual_gap, dense_qp_ipm_arg_ptr);
  d_dense_qp_ipm_arg_set_reg_prim(&solver_settings_.reg_prim, dense_qp_ipm_arg_ptr);
  d_dense_qp_ipm_arg_set_reg_dual(&solver_settings_.reg_dual, dense_qp_ipm_arg_ptr);
  d_dense_qp_ipm_arg_set_warm_start(&solver_settings_.warm_start, dense_qp_ipm_arg_ptr);
  d_dense_qp_ipm_arg_set_pred_corr(&solver_settings_.pred_corr, dense_qp_ipm_arg_ptr);
  d_dense_qp_ipm_arg_set_cond_pred_corr(&solver_settings_.cond_pred_corr, dense_qp_ipm_arg_ptr);
  int scale = solver_settings_.scale;
  d_dense_qp_ipm_arg_set("scale", &scale, dense_qp_ipm_arg_ptr);
  d_dense_qp_ipm_arg_set_kkt_fact_alg(&solver_settings_.kkt_fact_alg, dense_qp_ipm_arg_ptr);
  d_dense_qp_ipm_arg_set_remove_lin_dep_eq(&solver_settings_.remove_lin_dep_eq, dense_qp_ipm_arg_ptr);
  d_dense_qp_ipm_arg_set_compute_obj(&solver_settings_.compute_obj, dense_qp_ipm_arg_ptr);
  d_dense_qp_ipm_arg_set_split_step(&solver_settings_.split_step, dense_qp_ipm_arg_ptr);
}


void DenseQpIpmSolver::resize(const DenseQp& dense_qp) {
  dim_.resize(dense_qp);
  d_dense_qp_dim* dense_qp_dim_ptr = wrapper_holder_->dense_qp_dim_wrapper->get();
  ...
  // resize hpipm data
  wrapper_holder_->dense_qp_wrapper.resize(wrapper_holder_->dense_qp_dim_wrapper);
  wrapper_holder_->dense_qp_sol_wrapper.resize(wrapper_holder_->dense_qp_dim_wrapper);
  wrapper_holder_->dense_qp_ipm_ws_wrapper.resize(wrapper_holder_->dense_qp_dim_wrapper, 
                                                wrapper_holder_->dense_qp_ipm_arg_wrapper);
}


HpipmStatus DenseQpIpmSolver::solve(DenseQp& dense_qp, DenseQpSolution& qp_sol) {
  // resize data
  resize(dense_qp);
  ...
  if (solver_settings_.warm_start) {
    if (qp_sol.v.size() != dim_.nv) {
      throw std::runtime_error("qp_sol.v.size() must be " + std::to_string(dim_.nv));
    }
  } else {
  ...
  d_dense_qp_set_all(H_ptr_, g_ptr_, A_ptr_, b_ptr_, 
                    idxb_ptr_, lb_ptr_, ub_ptr_, 
                    C_ptr_, lg_ptr_, ug_ptr_, 
                    Zl_ptr_, Zu_ptr_, zl_ptr_, zu_ptr_, 
                    idxs_ptr_, ls_ptr_, us_ptr_, dense_qp_ptr);
  ...
  // Solve QP
  d_dense_qp_dim* dense_qp_dim_ptr = wrapper_holder_->dense_qp_dim_wrapper->get();
  d_dense_qp_sol* dense_qp_sol_ptr = wrapper_holder_->dense_qp_sol_wrapper.get();
  d_dense_qp_ipm_arg* dense_qp_ipm_arg_ptr = wrapper_holder_->dense_qp_ipm_arg_wrapper->get();
  d_dense_qp_ipm_ws* dense_qp_ipm_ws_ptr = wrapper_holder_->dense_qp_ipm_ws_wrapper.get();
  ...
  // Solve the QP
  d_dense_qp_ipm_solve(dense_qp_ptr, dense_qp_sol_ptr, dense_qp_ipm_arg_ptr, dense_qp_ipm_ws_ptr);
  ...
  d_dense_qp_ipm_get_max_res_stat(dense_qp_ipm_ws_ptr, &solver_statistics_.max_res_stat);
  d_dense_qp_ipm_get_max_res_eq(dense_qp_ipm_ws_ptr, &solver_statistics_.max_res_eq);
  d_dense_qp_ipm_get_max_res_ineq(dense_qp_ipm_ws_ptr, &solver_statistics_.max_res_ineq);
  d_dense_qp_ipm_get_max_res_comp(dense_qp_ipm_ws_ptr, &solver_statistics_.max_res_comp);
  ...
  for (int i=0; i<=solver_statistics_.iter+1; ++i) {
    if (i*stat_m < dense_qp_ipm_ws_ptr->stat_max) {
      solver_statistics_.alpha_aff.push_back(dense_qp_ipm_ws_ptr->stat[stat_m*i+0]);
      solver_statistics_.mu_aff.push_back(dense_qp_ipm_ws_ptr->stat[stat_m*i+1]);
      solver_statistics_.sigma.push_back(dense_qp_ipm_ws_ptr->stat[stat_m*i+2]);
      solver_statistics_.alpha_prim.push_back(dense_qp_ipm_ws_ptr->stat[stat_m*i+3]);
      solver_statistics_.alpha_dual.push_back(dense_qp_ipm_ws_ptr->stat[stat_m*i+4]);
      solver_statistics_.mu.push_back(dense_qp_ipm_ws_ptr->stat[stat_m*i+5]);
      solver_statistics_.res_stat.push_back(dense_qp_ipm_ws_ptr->stat[stat_m*i+6]);
      solver_statistics_.res_eq.push_back(dense_qp_ipm_ws_ptr->stat[stat_m*i+7]);
      solver_statistics_.res_ineq.push_back(dense_qp_ipm_ws_ptr->stat[stat_m*i+8]);
      solver_statistics_.res_comp.push_back(dense_qp_ipm_ws_ptr->stat[stat_m*i+9]);
      solver_statistics_.obj.push_back(dense_qp_ipm_ws_ptr->stat[stat_m*i+10]);
      solver_statistics_.lq_fact.push_back(dense_qp_ipm_ws_ptr->stat[stat_m*i+11]);
      solver_statistics_.itref_pred.push_back(dense_qp_ipm_ws_ptr->stat[stat_m*i+12]);
      solver_statistics_.itref_corr.push_back(dense_qp_ipm_ws_ptr->stat[stat_m*i+13]);
      solver_statistics_.lin_res_stat.push_back(dense_qp_ipm_ws_ptr->stat[stat_m*i+14]);
      solver_statistics_.lin_res_eq.push_back(dense_qp_ipm_ws_ptr->stat[stat_m*i+15]);
      solver_statistics_.lin_res_ineq.push_back(dense_qp_ipm_ws_ptr->stat[stat_m*i+16]);
      solver_statistics_.lin_res_comp.push_back(dense_qp_ipm_ws_ptr->stat[stat_m*i+17]);
    }
  }
...
const DenseQpIpmSolverStatistics& DenseQpIpmSolver::getSolverStatistics() const {
