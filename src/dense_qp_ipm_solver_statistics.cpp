...
void DenseQpIpmSolverStatistics::resize(const size_t size) {
...
void DenseQpIpmSolverStatistics::reserve(const size_t size) {
...
void DenseQpIpmSolverStatistics::clear() {
  alpha_aff.clear();
  mu_aff.clear();
  sigma.clear();
  alpha_prim.clear();
...
void DenseQpIpmSolverStatistics::disp(std::ostream& os) const {
  os << "================== Hpipm Dense QP Solver Statistics ==================" << std::endl;
  os << "status: " << (status == 0 ? "success" : (status == 1 ? "max iterations" : "min step length")) << std::endl;
  ...
  os << "dual_gap:     " << dual_gap << std::endl;
  os << "obj_value:    " << obj_value << std::endl;
  os << "alpha_aff    " 
     << "mu_aff       " 
     << "sigma        "
     << "alpha_prim   " 
