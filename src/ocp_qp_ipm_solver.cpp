...
void OcpQpIpmSolver::setSolverSettings(
    const OcpQpIpmSolverSettings& solver_settings) {
    ...
    d_ocp_qp_ipm_arg_set_iter_max(&solver_settings_.iter_max,
                                  ocp_qp_ipm_arg_ptr);
    d_ocp_qp_ipm_arg_set_alpha_min(&solver_settings_.alpha_min,
                                   ocp_qp_ipm_arg_ptr);
    d_ocp_qp_ipm_arg_set_mu0(&solver_settings_.mu0, ocp_qp_ipm_arg_ptr);
    d_ocp_qp_ipm_arg_set_tol_stat(&solver_settings_.tol_stat,
                                  ocp_qp_ipm_arg_ptr);
    d_ocp_qp_ipm_arg_set_tol_eq(&solver_settings_.tol_eq, ocp_qp_ipm_arg_ptr);
    d_ocp_qp_ipm_arg_set_tol_ineq(&solver_settings_.tol_ineq,
                                  ocp_qp_ipm_arg_ptr);
    d_ocp_qp_ipm_arg_set_tol_comp(&solver_settings_.tol_comp,
                                  ocp_qp_ipm_arg_ptr);
    d_ocp_qp_ipm_arg_set_reg_prim(&solver_settings_.reg_prim,
                                  ocp_qp_ipm_arg_ptr);
    d_ocp_qp_ipm_arg_set_warm_start(&solver_settings_.warm_start,
                                    ocp_qp_ipm_arg_ptr);
    d_ocp_qp_ipm_arg_set_pred_corr(&solver_settings_.pred_corr,
                                   ocp_qp_ipm_arg_ptr);
    d_ocp_qp_ipm_arg_set_ric_alg(&solver_settings_.ric_alg, ocp_qp_ipm_arg_ptr);
    d_ocp_qp_ipm_arg_set_split_step(&solver_settings_.split_step,
                                    ocp_qp_ipm_arg_ptr);
}
...
void OcpQpIpmSolver::resize(const std::vector<OcpQp>& ocp_qp) {
    ...
    d_ocp_qp_dim_set_all(dim_.nx.data(), dim_.nu.data(), dim_.nbx.data(),
                         dim_.nbu.data(), dim_.ng.data(), dim_.nsbx.data(),
                         dim_.nsbu.data(), dim_.nsg.data(),
    ...
    wrapper_holder_->ocp_qp_sol_wrapper.resize(
        wrapper_holder_->ocp_qp_dim_wrapper);
    wrapper_holder_->ocp_qp_ipm_ws_wrapper.resize(
        wrapper_holder_->ocp_qp_dim_wrapper,
...
HpipmStatus OcpQpIpmSolver::solve(const Eigen::VectorXd& x0,
    ...
    for (int i = 0; i <= dim_.N; ++i) {
        if (solver_settings_.warm_start) {
            if (qp_sol[i].x.size() != dim_.nx[i]) {
                throw std::runtime_error("qp_sol[" + std::to_string(i) +
                                         "].x.size() must be " +
                                         std::to_string(dim_.nx[i]));
            }
        } else {
    ...
    d_ocp_qp_set_all(A_ptr_.data(), B_ptr_.data(), b_ptr_.data(), Q_ptr_.data(),
                     S_ptr_.data(), R_ptr_.data(), q_ptr_.data(), r_ptr_.data(),
                     idxbx_ptr_.data(), lbx_ptr_.data(), ubx_ptr_.data(),
                     idxbu_ptr_.data(), lbu_ptr_.data(), ubu_ptr_.data(),
                     C_ptr_.data(), D_ptr_.data(), lg_ptr_.data(),
                     ug_ptr_.data(), Zl_ptr_.data(), Zu_ptr_.data(),
                     zl_ptr_.data(), zu_ptr_.data(), idxs_ptr_.data(),
                     lls_ptr_.data(), lus_ptr_.data(), ocp_qp_ptr);
    ...
    d_ocp_qp_ipm_arg* ocp_qp_ipm_arg_ptr =
        wrapper_holder_->ocp_qp_ipm_arg_wrapper->get();
    d_ocp_qp_ipm_ws* ocp_qp_ipm_ws_ptr =
        wrapper_holder_->ocp_qp_ipm_ws_wrapper.get();
    if (solver_settings_.warm_start) {
        for (int i = 0; i < dim_.N; ++i) {
    ...
    d_ocp_qp_ipm_solve(ocp_qp_ptr, ocp_qp_sol_ptr, ocp_qp_ipm_arg_ptr,
                       ocp_qp_ipm_ws_ptr);
    ...
    for (int i = 0; i < dim_.N; ++i) {
        ...
        d_ocp_qp_ipm_get_ric_P(ocp_qp_ptr, ocp_qp_ipm_arg_ptr,
                               ocp_qp_ipm_ws_ptr, i + 1,
                               qp_sol[i + 1].P.data());
        d_ocp_qp_ipm_get_ric_p(ocp_qp_ptr, ocp_qp_ipm_arg_ptr,
                               ocp_qp_ipm_ws_ptr, i + 1,
                               qp_sol[i + 1].p.data());
        d_ocp_qp_ipm_get_ric_K(ocp_qp_ptr, ocp_qp_ipm_arg_ptr,
                               ocp_qp_ipm_ws_ptr, i, qp_sol[i].K.data());
        d_ocp_qp_ipm_get_ric_k(ocp_qp_ptr, ocp_qp_ipm_arg_ptr,
                               ocp_qp_ipm_ws_ptr, i, qp_sol[i].k.data());
    }
    ...
    // Feedback gain
    Lr0_.setZero();
    d_ocp_qp_ipm_get_ric_Lr(ocp_qp_ptr, ocp_qp_ipm_arg_ptr, ocp_qp_ipm_ws_ptr,
                            0, Lr0_.data());
    Lr0_inv_.noalias() =
        (Lr0_.template triangularView<Eigen::Lower>())
            .solve(Eigen::MatrixXd::Identity(dim_.nu[0], dim_.nu[0]));
    ...
    d_ocp_qp_ipm_get_max_res_stat(ocp_qp_ipm_ws_ptr,
                                  &solver_statistics_.max_res_stat);
    d_ocp_qp_ipm_get_max_res_eq(ocp_qp_ipm_ws_ptr,
                                &solver_statistics_.max_res_eq);
    d_ocp_qp_ipm_get_max_res_ineq(ocp_qp_ipm_ws_ptr,
                                  &solver_statistics_.max_res_ineq);
    d_ocp_qp_ipm_get_max_res_comp(ocp_qp_ipm_ws_ptr,
